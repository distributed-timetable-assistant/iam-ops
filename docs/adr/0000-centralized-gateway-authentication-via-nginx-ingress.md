...
- **Kratos-Auth** (Custom Express/Node.js UI Frontend): `identity.outi.ir`
...
4. **User Authentication Interface**: The browser lands on the `oauth2-proxy` login initializing handler, which forwards the user to the `kratos-auth` platform running at `identity.outi.ir`.
...
- **Centralized UI Management**: Using `kratos-auth` at `identity.outi.ir` separates the core IAM engine logic from visual templates, enabling quick UI adjustments without patching core security deployments.
